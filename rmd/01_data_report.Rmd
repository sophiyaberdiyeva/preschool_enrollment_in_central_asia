...
### 1.1. Loading the preprocessed individual data
...
* Missing values are present for five variables, including the outcome. We propose the following strategies for handling this issue:

  - *EC12.	Child speaks using sentences of 5 or more words that go together*:	397 missing values imputed by "No".
  
  - *UB8.	Currently attending early childhood education programme*: 879 (30.52%) missing values. We will recode them to "No", since missing values are simply the result of filtering out children that have never attended preschool (which is a separate item ED4)
  ...
  - *ED5A.	Highest level of education attended (by caregiver)*:	3 missing values - omit rows containing them.
...
* There are no data on social transfers for this country
...
* Here, 6 variables have missing values (excluding social transfers-related set of items):
  ...
  - *ED5A.	Highest level of education attended (by caregiver)*:	1 missing value - omit row containing it.
...
* Missing values are present for five variables (again, except for social transfers). We propose the following strategies for handling this issue:
  ...
  - *ED4.	Ever attended educational institution*:	1 missing value, row will be omitted.
...
* Again, 5 variables have missing values:

  - *EC27.	A child speak using sentences of 5 or more words that go together*: 20 missing values will be recoded as "No".
  
  - *UB7.	Attended early childhood education programme of current school year*:	379 (49.03%)) are missing, and sample is even smaller - 773 observations. We plan to recode them to "No" as well due to the same filtering roots of the problem. *UB8	Currently attending early childhood education programme* is half missing too (50.97%), but it doesn't concern us much in this case.
  
  - *HL17.	Does natural father live in the household*: 5 missing values - omit rows containing them.
  
  -   - *ED5A.	Highest level of education attended (by caregiver)*:	2 missing values - omit rows containing them.

* Information on social transfers were not included into the second round of data collection (i.e. subset we are currently using), which makes the items being present in the full datafile, but entirely empty in ours.
...
* Among all variables related to child discipline (HCDI3[A-H]), 24 values are missing. Respective rows will be later omitted.
...
results <- pairwise_chi_cramers_parallel(
  kazakhstan_mics[, !names(kazakhstan_mics) %in% c("LN", "UF1", "UF2","UF3", "UF4","UF10", "UF17","line_num_index", "house_index", "HH1", "HL1")], n_cores = 4, p_threshold = 0.05, use_bonferroni = FALSE)
...
As we can observe, only 40 (unique) pairs had associations strength higher than 0.5:
...
* As expected, items measuring one phenomena are associated with each other: we can see clear colored clusters for Child Discipline UCD2[A-H] group of variables, for playthings materials EC2[A-C], EC5[A-F] - early stimulation and responsive care set, and the most clear one for the EC6-EC25, i.e. ECDI2030. All of these groups will be consolidated later, so these associations should not be problematic.
...
* HH48 (number of household members) seems to be associated with other household characteristics like number of children under 5 (HH51), wealth (windex5), having vehicles like a motorcycle or a car, and mother/father of the child living in the household (HL13/HL17, respectively), and also with early stimulation and responsive care (EC5[A-F]).

* **The most concerning finding: few (just 9, without any household-related features) pairwise associations between target variable "UB8" and the rest of predictors**. If this will also be the reflected in the multilevel modelling results, we will consider selecting out the component items and using them instead of composite scores (e.g. separate early childhood development items instead of ECDI2030).
